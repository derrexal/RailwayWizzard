...
public class GetTrainInformationService: BaseGetDataService, IGetTrainInformationService
{
    ...
    {
        var (departureStationCode, arrivalStationCode, departureDateTime, getTrainsFromSchedule) = request;

        var url = $"https://ticket.rzd.ru/api/v1/railway-service/prices/train-pricing?" +
                  $"service_provider=B2B_RZD&" +
                  $"getByLocalTime=true&" +
                  $"carGrouping=DontGroup&" +
                  $"origin={departureStationCode}&" +
                  $"destination={arrivalStationCode}&" +
                  $"departureDate={departureDateTime:yyyy-MM-ddT00:00:00}&" +
                  $"specialPlacesDemand=StandardPlacesAndForDisabledPersons&" +
                  $"carIssuingType=Passenger&" +
                  $"getTrainsFromSchedule={getTrainsFromSchedule.ToString().ToLower()}&" +
        ...
        requestMessage.Headers.Add("Cookie", " LANG_SITE=ru;");
